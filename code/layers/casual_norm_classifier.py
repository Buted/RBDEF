...
class Causal_Norm_Classifier(nn.Module):
    r"""
    code from https://github.com/KaihuaTang/Long-Tailed-Recognition.pytorch/blob/master/classification/models/CausalNormClassifier.py
    """
    def __init__(self, num_classes=1000, feat_dim=2048, use_effect=True, num_head=4, tau=16.0, alpha=3.0, gamma=0.03125):
    ...
    def forward(self, x, embed):
        normed_w, normed_x, y = self.train_forward(x)

        # remove the effect of confounder c during test
        if (not self.training) and self.use_effect:
            embed = embed.float().view(1, -1).to(x.device)
            normed_c = self.multi_head_call(self.l2_norm, embed)
